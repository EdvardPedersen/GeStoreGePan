...
#End of changes
    ...
    if($self->{'database'}->getDatabaseTaxon() eq 'gestore') {
        ...
        my $db_name = $paths[-1];
        $statement = 'echo "Getting db for blast" >> $timeFile'."\n";
        $statement = '(time hadoop jar '.GESTORE_PATH.' org.diffdb.move -D file='.$db_name.' -D run='.$self->{'run'}.' -D type=r2l -D regex='.$self->{'regex'}.' -conf='.GESTORE_CONFIG.') 2>> $timeFile'."\n";
        
        $statement .= "rm newBlastResult\n";
        # Do the blast
        $statement .= 'echo "Doing blast run" >> $timeFile'."\n";
        $statement .= '(time '.BLAST_PATH." -p ".$self->{'program'}." -d $db_name -i ".$self->{'input_file'}." -o newBlastResult -b 5 -m 8".') 2>> $timeFile'."\n";
        $statement .= "touch newBlastResult\n";        
        ...
        # Get previous result
        $statement .= 'echo "Retrieving last result for blast" >> $timeFile'."\n";
        $statement .= '(time hadoop jar '.GESTORE_PATH.' org.diffdb.move -D full_run=true -D file=blast_result_'.$db_name.' -D task=${SGE_TASK_ID} -D run='.$self->{'run'}.' -D type=r2l -conf='.GESTORE_CONFIG.') 2>> $timeFile'."\n";
        ...
        $statement .= 'echo "Putting new results into gestore" >> $timeFile'."\n";
        $statement .= '(time hadoop jar '.GESTORE_PATH.' org.diffdb.move -D file=blast_result_'.$db_name.' -D task=${SGE_TASK_ID} -D path=blastRes -D run='.$self->{'run'}.' -D type=l2r -conf='.GESTORE_CONFIG.') 2>> $timeFile'."\n";
