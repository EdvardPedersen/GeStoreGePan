...
use GePan::Config qw(PERL_PATH GEPAN_PATH NODE_LOCAL_PATH GESTORE_PATH GESTORE_CONFIG PYTHON_PATH);
...
sub _printGeStoreCall{
    ...
    my $hadoopLine = "hadoop jar ".GESTORE_PATH." org.gestore.move ";
    my $fileLine = "-D file=".$args->{'filename'}." ";
    my $runLine = "-D run=".$args->{'run'}." ";
    my $typeLine = "-D type=".$args->{'type'}." -D full_run=true ";
    ...
    if($args->{'quick'}) {
        $quickLine = " -D quick_add=true ";
    }

    print $file "(time ".$hadoopLine.$fileLine.$runLine.$pathLine.$taskLine.$typeLine.$formatLine.$quickLine.$confLine.') 2>> $timeFile'."\n";
}
...
sub _printArrayShell{
    ...
    print IN "cd ".NODE_LOCAL_PATH.'/gepan/${JOB_ID}'.'_'.'${SGE_TASK_ID}'."\n";
	...
	while(my $db_config = $dbs->getNextElement()){  
            ...
            if($params->{'gestore'}) {
                ...
                #my $db_statement = 'hadoop jar '.GESTORE_PATH.' org.gestore.move -D file='.$params->{'script_id'}.$db_name.' -D run='.$toolParams->{'run'}.' -D type=r2l -D regex='.$toolParams->{'regex'}."-conf=".GESTORE_CONFIG."\n";
		_printGeStoreCall( *IN, {'filename'=>$params->{'script_id'}.$db_name, 'type'=>"r2l", 'run'=>$toolParams->{'run'}, 'regex'=>$toolParams->{'regex'}});
                #print IN $db_statement."\n";
            }
            eval{
...
sub _printSchedulerShell{
    ...
    if($params->{'gestore'})
    {
        ...
        print IN "do\n";
        print IN "\t";
        #for(my $number = 0; $number < $params->{'queueing'}; $number++) {
        ...
        print IN "done\n";
        print IN "sleep 20";
        print IN "until /home/epe005/bin/mr_check; do :; done > /dev/null\n";
