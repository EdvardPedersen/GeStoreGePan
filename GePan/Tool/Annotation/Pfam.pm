...
sub _getExecuteStatement{
    ...
    if($self->{'database'}->getDatabaseTaxon() eq 'gestore') {
      ...
      my $db_name = $paths[-1];
      $statement = 'echo "Getting db for pfam" >> $timeFile'."\n";
      ...
      $statement .= "touch pfamOutput\n";
      $statement .= 'echo "Getting previous result" >> $timeFile'."\n";
      $statement .= '(time hadoop jar '.GESTORE_PATH.' org.diffdb.move -D full_run=true -D file=pfam_result_'.$db_name.' -D task=${SGE_TASK_ID} -D run='.$self->{'run'}.' -D type=r2l -conf='.GESTORE_CONFIG.') 2>> $timeFile'."\n";
      ...
      $statement .= 'echo "Putting new results in GeStore" >> $timeFile'."\n";
